...
Camera2D camera = Camera2D{.offset = Vector2{10, 10},
                           .target = Vector2{0, 0},
                           .rotation = 0,
                           .zoom = 10.0f};
...
void DrawPlayfield(const Playfield& playfield) {
  ...
  SetShaderValueTexture(_playfieldShader, _cellPropertyShaderLoc,
                        _cellPropertyTex);
  DrawTexturePro(
      _playfieldTex,
...
#pragma omp parallel for
  for (int y = 0; y < imH; y++) {
    for (int x = 0; x < imW; x++) {
      ...
      if (cell) {
        ...
        // in pixel space, but could be in world space
  ...
  PAB_INFO("GenPlayfieldImage took %d ms to take %d samples", elapsedMs,
           imW * imH);
  Image pfImage = {.data = pixels,
                   .width = imW,
                   .height = imH,
                   .mipmaps = 1,
...
void Init() {
  ...
  SetShaderValueV(_playfieldShader, paletteLoc, paletteData,
                  SHADER_UNIFORM_VEC4, 256);
...
void Tick() {
  ...
  if (modechanged) {
    ...
    SetShaderValue(_playfieldShader, modeLoc, &_playfieldShaderMode,
                   SHADER_UNIFORM_INT);
  }
...
void UpdateInterpolation(float dt) {
  ...
  // _snapshotBuffer[1] is the next tick (i.e. the new/current tick to
  // interpolate towards)
