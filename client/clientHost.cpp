...
static void ParsePacket(std::vector<uint8_t>& data) {
  ...
  data.erase(
      data.begin(),
      data.begin() +
          HEADER_SIZE); // After this you may NOT keep using PacketReader pr.
...
static void ClientProcessEvents(ENetHost* host, bool* running) {
  ...
  while (enet_host_service(host, &event, enetWaitTimeMs) > 0) {
    ...
    case ENET_EVENT_TYPE_RECEIVE: {
      ...
      std::vector<uint8_t> packetAsVec(
...
static ENetPeer* ConnectToServer(ENetHost* clientHost, const std::string& ip,
                                 int port) {
  ENetAddress address = {0};
  enet_address_set_host(&address, ip.c_str());
  address.port = port;
  const uint32_t connectionWaitTimeMs = 5000;
  PAB_INFO("Connecting to server at %s:%d with %d ms timeout...", ip.c_str(),
           port, connectionWaitTimeMs);
  ...
  while (!WindowShouldClose() && running) {
    ...
    if (Millis() - tickTimeStamp > tickPeriodMs) {
      ...
      while (auto packetDataOpt = pab::client::ConsumePacketToSend()) {
        ENetPacket* packet = enet_packet_create((*packetDataOpt).data(),
                                                (*packetDataOpt).size(), 0);
