...
static void CleanUpDisconnectedClient(ENetPeer* peer) {
  if (!gPeerToPlayerId.contains(peer)) {
    return;
  }
...
static void ServerProcessEvents(ENetHost* serverHost, uint32_t waitMs) {
  ...
  while (res > 0) {
    ...
    case ENET_EVENT_TYPE_CONNECT: {
      PAB_INFO("New client connected from %s",
               FormatIp(&event.peer->address.host));
    ...
    if (Millis() - tickTimeStamp > tickPeriodMs) {
      ...
      while (auto packetOpt = pab::server::ConsumePacketToSend()) {
        ...
        if (cmd < (uint8_t)Command::COMMAND_COUNT) {
          packetFlags =
              CommandRegistry[cmd].isReliable ? ENET_PACKET_FLAG_RELIABLE : 0;
        } else {
          PAB_ERR("No command registry entry for command %d", cmd);
        }
        ENetPacket* packet =
            enet_packet_create(data.data(), data.size(), packetFlags);
        if (targetIdOpt.has_value()) {
          ...
          if (gPlayerIdToPeer.contains(targetPlayerId)) {
            ...
            PAB_WARN("No client for player id %d to send packet to",
                     targetPlayerId);
